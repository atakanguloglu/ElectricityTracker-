    ...
    public class SuperAdminController : Common.BaseController
    {
        ...
        public ActionResult<SystemResourcesDto> GetSystemResources()
        {
            try
            {
                ...
                var databaseConnections = GetDatabaseConnections();
                var activeSessions = GetActiveSessions();
                
                var resources = new SystemResourcesDto
                {
                    CpuUsage = (int)Math.Round(cpuUsage, 0),
                    MemoryUsage = (int)Math.Round(memoryUsage, 0),
                    DiskUsage = diskUsage,
                ...
                _logger.LogInformation("System resources retrieved: CPU={CpuUsage}%, RAM={MemoryUsage}%, Disk={DiskUsage}%", 
                    resources.CpuUsage, resources.MemoryUsage, resources.DiskUsage);

                return Ok(resources);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to retrieve system resources");
        ...
        private double GetCpuUsage()
        {
            try
            {
                // Sistem CPU kullanımını hesapla
                var startTime = DateTime.Now;
                
                // İlk CPU zamanını al
                var startCpuTime = TimeSpan.Zero;
                ...
                Thread.Sleep(500); // Daha uzun bekleme
                
                var endTime = DateTime.Now;
                
                // İkinci CPU zamanını al
                var endCpuTime = TimeSpan.Zero;
                foreach (var process in processes)
                {
                    try 
                    { 
                ...
                var totalCpuUsedMs = (endCpuTime - startCpuTime).TotalMilliseconds;
                var totalMsPassed = (endTime - startTime).TotalMilliseconds;
                
                // CPU kullanımını hesapla (çekirdek sayısına göre normalize et)
                var cpuUsage = Math.Round((totalCpuUsedMs / (totalMsPassed * processorCount)) * 100, 2);
                
                // Gerçekçi değerler için sınırla
                cpuUsage = Math.Min(cpuUsage, 95.0);
                cpuUsage = Math.Max(cpuUsage, 5.0);
                
                _logger.LogDebug("CPU usage calculated: {CpuUsage}% (cores: {ProcessorCount})", cpuUsage, processorCount);
                
                return cpuUsage;
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "CPU usage measurement failed, using fallback");
        ...
        private double GetMemoryUsage()
        {
            try
            {
                // Sistem toplam RAM kullanımını hesapla
                var memoryInfo = GC.GetGCMemoryInfo();
                
                // Toplam fiziksel RAM
                var totalPhysicalMemory = memoryInfo.TotalAvailableMemoryBytes;
                
                // Kullanılan RAM (toplam - kullanılabilir)
                var availableMemory = memoryInfo.TotalAvailableMemoryBytes;
                var usedMemory = totalPhysicalMemory - availableMemory;
                
                // Eğer hesaplama yanlışsa, alternatif yöntem
                if (usedMemory <= 0)
                {
                    // Tüm process'lerin RAM kullanımını topla
                    var processes = Process.GetProcesses();
                    var totalUsedMemory = processes.Sum(p => {
                ...
                var memoryUsage = Math.Round((double)usedMemory / totalPhysicalMemory * 100, 2);
                
                // Gerçekçi değerler için sınırla
                memoryUsage = Math.Min(memoryUsage, 95.0);
                memoryUsage = Math.Max(memoryUsage, 10.0);
                
                _logger.LogDebug("Memory usage calculated: {MemoryUsage}% ({UsedMemory}MB / {TotalMemory}MB)", 
                    memoryUsage, 
                    Math.Round(usedMemory / (1024.0 * 1024), 2),
                    Math.Round(totalPhysicalMemory / (1024.0 * 1024), 2));
                
                return memoryUsage;
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Memory usage measurement failed, using fallback");
        ...
        private int GetDiskUsage()
        {
            try
            {
                var systemPath = Path.GetPathRoot(Environment.SystemDirectory);
            ...
            {
                _logger.LogWarning(ex, "Disk usage measurement failed, using fallback");
