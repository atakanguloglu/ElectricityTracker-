...
export default function ApiManagementPage() {
  ...
  // Fetch tenants separately (only once on mount)
  useEffect(() => {
    fetchTenants();
  }, []);
  ...
  const fetchApiKeys = async () => {
    try {
      ...
      setApiKeys(data.items || data.Items || []);
      setPagination(prev => ({
        ...prev,
  ...
  const fetchTenants = async () => {
    try {
      ...
      console.log('Tenants items:', data.items || data.Items);
      setTenants(data.items || data.Items || []);
    } catch (err) {
  ...
  const tenantOptions = useMemo(() => 
    (tenants || []).map(tenant => (
      <Option key={tenant.id || tenant.Id || 0} value={tenant.id || tenant.Id || 0}>
        {tenant.companyName || tenant.CompanyName || 'Tenant'}
      </Option>
    )), [tenants]
  );
  ...
  const modalTenantOptions = useMemo(() => {
    console.log('Creating modalTenantOptions with tenants:', tenants);
  ...
  const stats = useMemo(() => [
    ...
    },
    {
      ...
      value: (apiKeys || []).filter(k => (k.status || k.Status) === 'active').length,
    ...
    },
    {
      ...
      value: (apiKeys || []).reduce((sum, key) => sum + (key.totalCalls || key.TotalCalls || 0), 0).toLocaleString(),
    ...
    },
    {
      ...
      value: (apiKeys || []).length > 0 
        ? `${((apiKeys || []).reduce((sum, key) => sum + (key.errorRate || key.ErrorRate || 0), 0) / (apiKeys || []).length * 100).toFixed(1)}%`
        : '0%',
  ...
  const filteredApiKeys = useMemo(() => {
    return (apiKeys || []).filter(key => {
      if (filters.tenantId && (key.tenantId || key.TenantId) !== filters.tenantId) return false;
      if (filters.status && (key.status || key.Status) !== filters.status) return false;
      if (filters.hasWebhook !== undefined) {
        const hasWebhook = (key.webhookUrl || key.WebhookUrl) !== null && (key.webhookUrl || key.WebhookUrl) !== undefined;
  ...
  const handleEdit = (apiKey: ApiKey) => {
    ...
    form.setFieldsValue({
      name: apiKey.name || apiKey.Name,
      tenantId: apiKey.tenantId || apiKey.TenantId,
      permissions: apiKey.permissions || apiKey.Permissions,
      rateLimit: apiKey.rateLimit || apiKey.RateLimit,
      rateLimitPeriod: apiKey.rateLimitPeriod || apiKey.RateLimitPeriod,
      webhookUrl: apiKey.webhookUrl || apiKey.WebhookUrl
    });
    setModalVisible(true);
  };

  const handleViewStats = async (apiKey: ApiKey) => {
    await fetchApiUsage(apiKey.id || apiKey.Id || 0);
    // Show stats in a modal or drawer
    message.info(`${apiKey.name || apiKey.Name || 'API Key'} için istatistikler yüklendi`);
  };
  ...
  const handleRegenerate = async (apiKey: ApiKey) => {
    try {
      ...
      setApiKeys(prev => prev.map(key => 
        (key.id || key.Id) === (apiKey.id || apiKey.Id) ? updatedApiKey : key
      ));
  ...
  const columns: ColumnsType<ApiKey> = [
    {
      ...
      render: (text, record) => (
        <div>
          <div style={{ fontWeight: 500 }}>{text || record.Name}</div>
          <div style={{ fontSize: '12px', color: '#666' }}>
            {(record.key || record.Key || '').substring(0, 8)}...{(record.key || record.Key || '').substring((record.key || record.Key || '').length - 8)}
          </div>
        </div>
      )
    },
    {
      ...
      key: 'tenantName',
      render: (text, record) => <Tag color="blue">{text || record.TenantName}</Tag>
    },
    {
      title: 'Durum',
      dataIndex: 'status',
      ...
      render: (status, record) => (
        <Badge 
          status={getStatusColor(status || record.Status || '') as any} 
          text={getStatusText(status || record.Status || '')} 
        />
      )
    },
    {
      ...
      render: (permissions, record) => (
        <Space>
          {(permissions || record.Permissions || '').split(',').map((perm: string, index: number) => (
    ...
    },
    {
      ...
      render: (_, record) => (
        <div>
          <div>{(record.rateLimit || record.RateLimit || 0)} / {(record.rateLimitPeriod || record.RateLimitPeriod || '')}</div>
        </div>
      )
    },
    {
      ...
      render: (_, record) => (
        <div>
          <div style={{ fontSize: '12px' }}>
            {(record.totalCalls || record.TotalCalls || 0).toLocaleString()} çağrı
          </div>
          <div style={{ fontSize: '12px', color: '#ff4d4f' }}>
            %{((record.errorRate || record.ErrorRate || 0) * 100).toFixed(1)} hata
          </div>
        </div>
      )
    },
    {
      ...
      render: (_, record) => (
        <div>
          {(record.webhookUrl || record.WebhookUrl) ? (
            <Badge 
              status={getWebhookStatusColor(record.webhookStatus || record.WebhookStatus || '') as any} 
              text={getWebhookStatusText(record.webhookStatus || record.WebhookStatus || '')} 
            />
          ) : (
      ...
      render: (lastUsed, record) => (
        <div>
          {(lastUsed || record.LastUsed) ? (
            <div style={{ fontSize: '12px' }}>
              {new Date(lastUsed || record.LastUsed || '').toLocaleDateString('tr-TR')}
            </div>
          ) : (
        ...
        <Space size="small">
          ...
          <Tooltip title="API Anahtarını Göster/Gizle">
            <Button 
              ...
              size="small"
              onClick={() => setShowApiKey(showApiKey === (record.id || record.Id || 0) ? null : (record.id || record.Id || 0))}
            />
          </Tooltip>
        ...
        <Modal
          ...
          footer={[
            <Button key="copy" type="primary" onClick={() => {
              const apiKey = apiKeys.find(k => (k.id || k.Id) === showApiKey);
              if (apiKey) copyToClipboard(apiKey.key || apiKey.Key || '');
            }}>
              Kopyala
            </Button>,
        ...
        >
          <div style={{ textAlign: 'center', padding: '20px' }}>
            <Text code style={{ fontSize: '16px', wordBreak: 'break-all' }}>
              {(apiKeys || []).find(k => (k.id || k.Id) === showApiKey)?.key || (apiKeys || []).find(k => (k.id || k.Id) === showApiKey)?.Key || ''}
            </Text>
