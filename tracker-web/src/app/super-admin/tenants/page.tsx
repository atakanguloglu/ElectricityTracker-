...
export default function TenantsPage() {
  ...
  const handleExport = (format: 'excel' | 'pdf' | 'csv') => {
    ...
    const exportData = tenantsData.items.map((t: Tenant) => ({
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              title="Aktif"
              value={(tenantsData?.items || []).filter((t: Tenant) => t.status === 'Active').length}
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              title="Askıda"
              value={(tenantsData?.items || []).filter((t: Tenant) => t.status === 'Suspended').length}
              prefix={<StopOutlined />}
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              title="Trial"
              value={(tenantsData?.items || []).filter((t: Tenant) => t.status === 'Trial').length}
